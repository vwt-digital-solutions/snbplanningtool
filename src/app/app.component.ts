...
export class AppComponent {
  ...
  refreshStatus : any = 'Auto refresh (5 min.)';
  refreshStatusClass : boolean = false;
  refreshStatusClasses() {
    return {
      small: true,
      error: this.refreshStatusClass
    }
  }
  ...
  private handleResult(result) {
    ...
    this.refreshUpdate = Date.now();
  };

  private handleError() {
    this.refreshStatusClass = true;
    this.refreshStatus = 'An error has occurred';
